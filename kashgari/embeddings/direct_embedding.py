...
class DirectEmbedding(Embedding):
    ...
    def __init__(self,
        ...
        """
        super(DirectEmbedding, self).__init__(task=task,
    ...
    def _build_model(self, **kwargs):
        if self.sequence_length == 0 or \
                self.sequence_length == 'auto':
            logging.debug('need to build after build_word2idx')
        else:
            input_tensor = L.Input(shape=(self.sequence_length, self.embedding_size,),
                                   name=f'input')

            #input_tensor =  L.Dense(self.embedding_size, input_shape=(self.embedding_size,)),
            #layer_embedding = L.Embedding(self.token_count,
            #                              self.embedding_size,
            #                              name=f'layer_embedding')

            #embedded_tensor = layer_embedding(input_tensor)
            self.embed_model = input_tensor
